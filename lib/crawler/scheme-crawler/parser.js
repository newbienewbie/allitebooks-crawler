...
class Parser {
    ...
    _parseArticleListItem(element) {
        ...
        // 缩略图
        let thumbnailUrl=cheerio(e).find("div.entry-thumbnail a img").attr("src");
        if(!thumbnailUrl.startsWith("http")){
            let _url=thumbnailUrl.startsWith("/")?thumbnailUrl:`/${thumbnailUrl}`;
            thumbnailUrl=`http://www.allitebooks.com${_url}`;
        }
